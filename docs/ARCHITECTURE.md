...
2. **Preview URL for Avital:** link the GitHub repo to a Vercel project so every branch push builds a preview URL she can open on her phone. Claude checks build logs and fetches the page through the Vercel MCP. Preview deployment protection is turned off for previews so no Vercel login is needed. _One-time manual step for Avital:_ install the Vercel GitHub App on the `avitalthehouseofbride` account (Vercel dashboard → Add New Project → Import from GitHub). Until then, Claude can push ad-hoc previews from the sandbox with the Vercel MCP `deploy_to_vercel`, so nothing is blocked.
